...
class StmDevice extends SerialDevice {
  ...
  /**
   * Flashes connected device with latest binary for CanOpener
  ...
  async flash(updateProgressBar) {
    ...
    try {
      let settings = { baudRate: this.baudRateSource(), parity: "even" };
      ...
      this.flashContent = await tools.readBinary();
      ...
      settings = { baudRate: this.baudRateSource() };
